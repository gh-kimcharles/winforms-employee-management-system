...
namespace SQLProject3 {
	...
	public:
		...
		LoginForm(Form^ call_etr) {
			InitializeComponent();
			start_up = call_etr;
			this->FormClosed += gcnew FormClosedEventHandler(this, &LoginForm::LoginForm_FormClosed);
		}
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->label2->Text = L"Username [ADMIN]:";
			// 
			// label3
			// 
			...
			this->img_login->ErrorImage = (cli::safe_cast<System::Drawing::Image^>(resources->GetObject(L"img_login.ErrorImage")));
...
#pragma endregion
	...
	private: System::Void btn_login_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		try {
			conn->Open();
			MySqlDataAdapter^ adapter = gcnew MySqlDataAdapter("SELECT adm_email, adm_password FROM tbl_admins", conn);
			DataSet^ ds = gcnew DataSet();
			adapter->Fill(ds, "tbl_admins");

			if (ds->Tables["tbl_admins"]->Rows->Count > 0) {
				for (int i{ 0 }; i < ds->Tables["tbl_admins"]->Rows->Count; i++) {
					DataRow^ row = ds->Tables["tbl_admins"]->Rows[i];
					String^ email_ = row["adm_email"]->ToString();
					String^ password_ = row["adm_password"]->ToString();

					if (email_->Equals(email, StringComparison::InvariantCultureIgnoreCase) && password_->Equals(password)) {
						is_login = true;
						MessageBox::Show("FILE: Admin login success.");
						break;
					}
				}
			}
			...
			if (is_login) {
				EmployeeTableRecord^ etr = gcnew EmployeeTableRecord();
				etr->Visible = true;
				Hide();
			}
			else {
